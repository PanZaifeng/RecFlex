...
class OneHotParams(NamedTuple):
    unroll_factor: int


class OneHotSchedule(ScheduleBase):
    def EmitScheduleBody(self, params: OneHotParams) -> str:
        meta = self.meta
  ...
  onehot::GatherSegments<{vars_.EMBED_DIM_SUM}, {meta.embed_dim}, {params.unroll_factor}, {self.block_threads}>(
      ...
      *reinterpret_cast<{self.EmitShmemType(params)} *>({vars_.shmem}),
      ...
      // num_inputs
      num_inputs,
      // task_bid, task_blocks(=1), num_tiles
      {vars_.task_bid}, 1, {vars_.num_tiles},
      // embed_dim_offset
      {vars_.embed_dim_offset});
'''
        return schedule_body

    def EmitShmemType(self, params: OneHotParams) -> str:
        return f"typename onehot::GatherSegmentsTempStorage<{self.block_threads}>"

    def EmitHostPreprocess(self, params: OneHotParams) -> str:
        meta = self.meta
        vars_ = self.pre_vars
        host = f'''
  onehot::OneHotOneBlock(
      // inputs
      {vars_.raw_data},
      // tile_size
      {self.block_threads},
...
'''
    ...
    def GenParamCandidates(self, stats: InputStats = None) -> List[OneHotParams]:
        unroll_factor_candidates = divisors(self.meta.embed_dim)
        return [OneHotParams(unroll_factor=unroll_factor)
