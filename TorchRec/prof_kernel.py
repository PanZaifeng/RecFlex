...
def nsys_profile(data_dir: str, table_config_path: str, rep_prefix_or_fname: str, e2e: bool,
                 gpu_id: int = None):
    if gpu_id is None:
        raise Exception("No vaild GPU provided!")

    cur_dir = os.path.dirname(os.path.abspath(__file__))
    script_path = os.path.join(cur_dir, "benchmark_torchrec.py")
    ...
    for model_name in args.model_names:
        ...
        if args.e2e:
            ...
            if not os.path.exists(result_path) or args.remeasure:
                task_manager.add_task(nsys_profile, (data_dir, table_config_path, result_path, args.e2e),
                                      ident_key="gpu_id")
        else:
            report_name = f"{report_prefix}.nsys-rep"
            if not os.path.exists(report_name) or args.remeasure:
                task_manager.add_task(nsys_profile, (data_dir, table_config_path, report_prefix, args.e2e),
                                      ident_key="gpu_id")
    task_manager.join()

    if args.e2e:
        with open(os.path.join(args.output_dir, "result_e2e.txt"), "w") as f:
            for model_name in args.model_names:
                e2e_path = os.path.join(args.output_dir, f"{model_name}_e2e.txt")
